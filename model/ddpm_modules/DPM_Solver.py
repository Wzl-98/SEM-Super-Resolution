...
class DPM_Solver:
    def __init__(
        ...
        self.noise_level_list = noise_level_list
    ...
    def data_prediction_fn(self, x, t, noise_level):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(x, noise_level)
        data = x[:, 1:2, :, :]
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t)
        x0 = (data - sigma_t * noise) / alpha_t

        if self.correcting_x0_fn is not None:
            x0 = self.correcting_x0_fn(x0, t)
        return x0

    def model_fn(self, x, t, noise_level):
        """
        Convert the model to the noise prediction model or the data prediction model.
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, t, noise_level)
        else:
            return self.noise_prediction_fn(x, noise_level)
    ...
    def sample(self, x, condition_x, noise_level_list, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
               method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
               atol=0.0078, rtol=0.05, return_intermediate=False,
               ):
        """
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                noise_level = torch.FloatTensor([noise_level_list[step]]).repeat(batch_size, 1).to(x.device)
                model_prev_list = [self.model_fn(torch.cat([condition_x, x], dim=1), t, noise_level)]
                if self.correcting_xt_fn is not None:
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(self.model_fn(torch.cat([condition_x, x], dim=1), t, noise_level))
                ...
                for step in range(order, steps + 1):
                    ...
                    if step < steps:
                        model_prev_list[-1] = self.model_fn(torch.cat([condition_x, x], dim=1), t, noise_level)
